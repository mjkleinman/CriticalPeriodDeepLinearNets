...
def main(args):
    ...
    for jj, deficit_start in enumerate(args.deficit_start_list):
        ...
        mcn = MultipathwayNet(8, 15, depth=depth, num_pathways=2, width=args.pathway_width, bias=False,
                              nonlinearity=nonlin, diagonal_init=True, U=U, V=V, eps=eps, kDiagInit=diag_values)
        ...
        for sing_value_idx in range(8):
            ...
            ax[jj].plot(k_traj[0], 'k--', linewidth='1', alpha=0.6, label='ODE' if sing_value_idx == 0 else None)
            ax[jj].plot(k_traj[1], 'k--', linewidth='1', alpha=0.6)
            ...
            leg.legendHandles[0].set_alpha(0.8)
    ...
    fig.tight_layout()
    fig.savefig(
        # f"development/analytical-and-sgd-depth={depth}-deficitLength={deficit_length}.pdf",
        f"plots/analytical-and-sgd-depth={depth}-deficitLength={deficit_length}.pdf",
